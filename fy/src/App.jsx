    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.addEventListener('message', (event) => {
        if (event.data && event.data.type === 'PLAY_SOUND') {
          ...
          // Attempt to play the audio; if blocked, you could set state to show a play button
          audio.play().catch((err) => {
            console.warn('Audio play was prevented due to autoplay policies:', err);
            // Optionally, update state to prompt user for manual sound playback
          });
        }
      });
    }
